...
SSHyClient.settings.prototype = {
  ...
  testShell: function (r) {
    ...
    if (
      r.substring(0, 2).indexOf("]") === -1 &&
      (this.shellString.length === 0 ||
        this.shellString.length > "]0;fish".length)
    ) {
      this.shellString = "";
      return;
    }
  ...
  setLocalEcho: function (dir) {
    ...
    if (this.localEcho === 1) {
      ...
      element.innerHTML =
        "State: " + (this.autoEchoState === false ? "Disabled" : "Enabled");
    } else {
  ...
  parseLocalEcho: function (r) {
    ...
    if (this.localEcho === 1) {
      ...
      if (!this.autoEchoState) {
        ...
        if (
          r.indexOf(this.fsHintEnter) != -1 ||
          r.toLowerCase().indexOf("password") != -1
        ) {
  ...
  parseKey: function (e) {
    ...
    if (
      e.key.length > 1 ||
      this.blockedKeys.includes(e.key) ||
      e.altKey ||
      e.ctrlKey ||
      e.metaKey
    ) {
  ...
  setColorScheme: function (colIndex) {
    ...
    document.getElementById("currentColor").innerHTML =
      themeName === undefined ? "Custom" : themeName;
  },
  ...
  importXresources: function () {
    ...
    reader.onload = function () {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        var result = lines[i].match(/#([0-9a-f]{3}){1,2}/gi);
        if (result) {
      ...
      colName =
        element.name === ".Xresources" ? "custom" : element.name.split(".")[0];
  ...
  cycleColorSchemes: function (dir) {
    ...
    if (
      this.colorCounter > this.colorNames.length - 1 ||
      this.colorCounter < 0
    ) {
