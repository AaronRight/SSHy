...
SSHyClient.Transport.prototype = {
  ...
  generate_key: function (char, size) {
    ...
    return this.preferred_kex.SHAVersion == "SHA-1"
      ? new SSHyClient.hash.SHA1(m.toString()).digest().substring(0, size)
      : new SSHyClient.hash.SHA256(m.toString()).digest().substring(0, size);
  },
  ...
  handle_dec: function (m) {
    ...
    try {
      ...
      console.log(err);
      console.log(
        "Error! code - " + m.substring(0, 1).charCodeAt(0) + " does not exist!"
      );
    }
  },
